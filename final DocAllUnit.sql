--SELECT * FROM dbo.M_ORG_T3 WHERE Code like '131%'
--Name like N'%اصف%' --Code='311'
declare @T3Id INT=0,@Year TINYINT=1,
    @SelectResult BIT = 1, 
    @Finalized BIT = 1
    --------------------------
     DECLARE
 ...
 DECLARE product_cursor CURSOR FOR   
    ...
    BEGIN  
		   ...
		   --set  @Year =1 
			-----------
			select
				@YYYY  = [dbo].[UF_PUB_GET_YYYY](@Year),
				@PRINTLOG  = CASE WHEN @SelectResult = 1 THEN 0 ELSE 1 END
